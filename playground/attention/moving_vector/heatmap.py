...
def main():
    ...
    checkpoint = torch.load("attention/moving_vector/coin_flip_att_model_step_2.pth")
    module.selection_mod = DynamicQueryAttention(5,12,["attr","v_latents"],2).cuda()
                ...
                for v in v_list:
                    ...
                    for i, corr_scale_attr in enumerate(corr_scales_attr):
                        ...
                        for j, corr_scale_v_latents in enumerate(corr_scales_v_latents):
                            ...
                            # Compute the accuracy and store it in the sum matrix for the current step
                            accuracy = correct / labels_indices.size(0)
                            accuracy_sum_matrix_per_step[step_limit][i, j] += accuracy
