...
def main():
    ...
    checkpoint = torch.load("attention/moving_vector/coin_flip_att_model_step_3.pth")
    ...
    # Define the scales
    corr_scales_v_latents = np.linspace(-0, .6, 20)
    corr_scales_attr = np.linspace(-0, .4, 20)
    ...
    with torch.no_grad():  # Disable gradient computation
        ...
        for val_batch in islice(val_loader, num_batches):
            ...
            for step_limit in range(0, module.selection_mod.n_steps + 1):
                ...
                # Iterate over each random vector v
                for v in v_list:
                    # Loop over corr_scale_attr
                    for i, corr_scale_attr in enumerate(corr_scales_attr):
                        # Loop over corr_scale_v_latents
                        for j, corr_scale_v_latents in enumerate(corr_scales_v_latents):
                            ...
                            # Accumulate the differences in the matrix
                            attention_diff_sum_matrix_per_step[step_limit][i, j] += attention_diff.mean().item()
