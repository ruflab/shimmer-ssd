...
def main():
    ...
    module.selection_mod = FixedSharedSelection()
    v = checkpoint['corruption_vector']
    ...
    accuracy_sum_matrix = np.zeros((100, 100))

    # Initialize a matrix to count the number of accuracies added per (i,j)
    accuracy_count_matrix = np.zeros((100, 100))

    # Define the scales
    corr_scales = np.linspace(-20, 20., 100)
    ...
    with torch.no_grad():  # Disable gradient computation
        ...
        for val_batch in islice(val_loader, 5):
            ...
            # Loop over corr_scale_attr and corr_scale_v_latents
            for i, corr_scale_attr in enumerate(corr_scales):
                for j, corr_scale_v_latents in enumerate(corr_scales):
                    ...
                    # Compute the accuracy and store it in the sum matrix
                    accuracy = correct / labels_indices.size(0)
                    accuracy_sum_matrix[i, j] += accuracy
    ...
    plt.tick_params(axis='both', which='major', labelsize=8)  # Adjust 'labelsize' to the desired size
    plt.xticks(np.arange(100), np.round(corr_scales, 2))
    plt.yticks(np.arange(100), np.round(corr_scales, 2))
