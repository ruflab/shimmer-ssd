...
def main():
    ...
    module.selection_mod = DynamicQueryAttention(10,12,["attr","v_latents"],3,.1).cuda()
    ...
    corr_scales_attr = np.linspace(-0, 2.5, 20)

    # Matrix loop for validation data transformation and attention computation
    with torch.no_grad():  # Disable gradient computation
        # Initialize matrices to accumulate the results across batches
        attention_diff_sum_matrix_per_step = {step: np.zeros((20, 20)) for step in range(0, module.selection_mod.n_steps + 1)}
        attention_count_matrix_per_step = {step: np.zeros((20, 20)) for step in range(0, module.selection_mod.n_steps + 1)}

        # Iterate over the batches using islice
            ...
            for step_limit in range(0, module.selection_mod.n_steps + 1):
                ...
                for i, corr_scale_attr in enumerate(corr_scales_attr):
                    for j, corr_scale_v_latents in enumerate(corr_scales_v_latents):
                        total_attention_diff = 0  # To accumulate the attention differences

                        for _ in range(40):  # Perform 10 random samples of v
                            # Random vector v
                            ...
                            encodings = module.encode(domain_encoded)[input_key]
                            
                            domain_encoded = {
                                'v_latents': domain_encoded[input_key]['v_latents'],
                                'attr': domain_encoded[input_key]['attr']
                            }

                            # Use the attention mechanism directly to get the attention scores
                            attention_scores = module.selection_mod.forward(encodings, domain_encoded)

                            # Extract attention scores for `v_latents` and `attr`
                            v_latents_attention = attention_scores['v_latents']  # Shape: (batch_size,)
                            attr_attention = attention_scores['attr']  # Shape: (batch_size,)

                            # Compute the difference: (attr - v_latents)
                            attention_diff = attr_attention - v_latents_attention

                            # Accumulate the differences across the 10 random samples of v
                            total_attention_diff += attention_diff.mean().item()

                        # Average the accumulated differences over the 10 random samples of v
                        avg_attention_diff = total_attention_diff / 40

                        # Accumulate the averaged attention differences in the matrix
                        attention_diff_sum_matrix_per_step[step_limit][i, j] += avg_attention_diff
                        attention_count_matrix_per_step[step_limit][i, j] += 1


    # Now let's plot the accumulated results as a heatmap
    ...
    # Compute the global max absolute difference
    max_abs_diff = max(
        np.abs(attention_diff_sum_matrix_per_step[step_limit] / attention_count_matrix_per_step[step_limit]).max()
        for step_limit in range(0, n_steps)
    )
    print("max_abs_diff : ",max_abs_diff)

    for step_limit in range(0, n_steps):
        # Compute the mean attention difference for this step across all batches
        attention_diff_matrix = attention_diff_sum_matrix_per_step[step_limit] / attention_count_matrix_per_step[step_limit]
        ...
        # Plot the attention difference matrix as a heatmap for this step limit in a subplot
        ax = axes[step_limit]  # Select the correct subplot
        im = ax.imshow(attention_diff_matrix, cmap="RdYlGn", origin="lower", aspect='auto', extent=extent, vmin=-max_abs_diff, vmax=max_abs_diff)  # Symmetric vmin/vmax
        fig.colorbar(im, ax=ax, label="Attention Difference (attr - v_latents)")
        ...
        ax.set_title(f"Attention Difference Heatmap (Step {step_limit})")
    ...
    plt.tight_layout()
    plt.savefig("attention_diff_heatmaps_steps_0_to_max.png")
    plt.show()
