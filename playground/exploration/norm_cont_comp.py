...
def main():
    ...
    lossv1 = contrastive_loss(
        gw_states_means1["v_latents"], gw_states_means2["v_latents"]
    )
    lossattr1 = contrastive_loss(
        gw_states_means1["attr"], gw_states_means2["attr"]
    )
    print(lossv1)
    print(lossattr1)
    perm = torch.randperm(2048).to(device)
    lossv2 = contrastive_loss(
        gw_states_means1["v_latents"], gw_states_means2["v_latents"][perm, :]
    )
    lossattr2 = contrastive_loss(
        gw_states_means1["attr"], gw_states_means2["attr"][perm, :]
    )
    print(lossv2)
    print(lossattr2)
